...
// The microphone audio has got the stream buffer 
  var audioStream = options.stream;
// creating audio context for the audio to get buffered
  var audioContext = options.context;
// Gaining the node for the spectrogram
  var gainNode = audioContext.createGain ? audioContext.createGain() : audioContext.createGainNode();

  gainNode.gain.value = 0.1;
// Oscillator for the spectrogram
  ...
  var timeout = null;
//Creation of the spectrogram UI
...
//Canvas context for uploading the UI of the canvas i.e Spectrogram
  var canvasContext = canvas.getContext("2d");

  canvasContext.fillStyle = background;
  canvasContext.fillRect(0, 0, width, height);
// Analyzer constructor is called
  var analyser = audioContext.createAnalyser();
  audioStream.connect(analyser);

  var moveBy = 1;
//Heat signature color
...
//Drawing the spectrogram by calling the function itself
  var draw = function() {
